...
class RentCastAPI:
    ...
    def use_region(self, region: APISearchRegion, **kwargs):
        self.region = region
        self.region_complete = False
    
    def get_listings_chunk(self):
        if (self.__request_count >= self.request_limit): raise Exception('Request limit reached, cannot continue with request')
